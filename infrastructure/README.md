...
The AWS infrastructure that runs [webhook-listener](../webhook-listener/README.md) and
[dispatch-worker](../dispatch-worker/README.md) as long-lived services, and
[specialist-runner](../specialist-runner/README.md) as an on-demand task, defined in TypeScript with
...
whenever their own configuration or pinned image tag changes. All three read the network's outputs
through S3 remote state; `temporal-workers` also reads `specialist-sandbox`'s, and `listener` reads
`temporal-workers`'s.
...
Two things this stack does not do, both parked and recorded in Known gaps below: egress is unrestricted
rather than allowlisted, and its secrets are read directly from SSM — the listener's own credentials,
under the same prefix — rather than through a credential-injection proxy.
...
The worker is [`dispatch-worker/`](../dispatch-worker). This stack passes it the `SPECIALIST_*` env vars
and the reviewer table (`REVIEWER_EMAIL_TO_GITHUB_LOGIN`, from `temporal.reviewer-email-to-github-login`),
and grants its task role the `ecs:RunTask`/`ecs:DescribeTasks`/`iam:PassRole` permission to dispatch
(see `constructs/temporal-worker-service.ts`'s `dispatchTarget` config). The trigger is on the app side
rather than in this stack: `webhook-listener`'s `specialist-dispatch` lane (`src/lanes/specialist-
dispatch.ts`, `src/dispatch-trigger.ts`) starts `dispatchStoryWorkflow` when a story enters `In Progress`
...
| ECR repository (specialist sandbox) | Same posture as the listener's — [`build-and-push-specialist-ecr.yml`](../.github/workflows/build-and-push-specialist-ecr.yml) creates and populates it from [`specialist-runner/`](../specialist-runner). On a fresh AWS account, run that workflow once (a merge to `main` under `specialist-runner/`, or a manual run) before applying `specialist-sandbox`. |
...
| ECR repository (Temporal worker) | Same posture as the other two — [`build-and-push-dispatch-worker-ecr.yml`](../.github/workflows/build-and-push-dispatch-worker-ecr.yml) creates and populates it from [`dispatch-worker/`](../dispatch-worker). On a fresh AWS account, run it once before applying `temporal-workers`. |
...
Every stack reads from one shared `parameter-prefix`, and `specialist-sandbox`'s three secret
...
listener read the identical `ANTHROPIC_API_KEY` value, not distinct ones (see Known gaps).
...
is issued fresh per deployment like any other credential here — not one account-level key. The
...
gitignored, same as the synth output directory — regenerate it after a fresh clone, same as `npm install`.
...
`domain-name` and `hosted-zone-id`, `resource-name-prefix`, `parameter-prefix`, `framework-repo`,
`output` if you want a different synth directory, and the `reviewer-email-to-github-login` table.
Nothing deploys until those are real. The template's key set is kept identical to a working config, so a missing key is a
...
| `resource-name-prefix` | `example` | Leading part of the network, cluster and load balancer names (e.g. the `example-prod` cluster) |
...
| `listener.log-retention-days` | `14` | |
...
| `specialist-sandbox.ecr-repository-name` | `intent-to-production-specialist` | Must match the CI workflow's `ECR_REPOSITORY` — see Prerequisites |
| `specialist-sandbox.image-tag` | `latest` | See Image tags below |
| `specialist-sandbox.cpu` / `.memory` | `2048` / `4096` | Task-level Fargate sizing. `1024`/`2048` is undersized for a real target repo's dependency install + build + test run, plus a Playwright browser download when the surface has browser tests. Fargate has no swap: exceeding the memory limit is a hard OOM kill of the whole task, not graceful degradation, and a killed task looks like an unexplained specialist failure with no clear error from the app itself. Raise further (e.g. `4096`/`8192`) if a real engagement's build/test suite is heavier than this |
| `specialist-sandbox.log-retention-days` | `14` | |
...
| `specialist-sandbox.framework-ref` | `dev` | Git ref of the framework repo to clone, as `FRAMEWORK_REF`. A pinned ref here controls what every specialist run in this deployment uses, independent of whatever ref this deployment's own listener/temporal-workers images were built from |
...
| `temporal.ecr-repository-name` | `intent-to-production-temporal-worker` | Must match the CI workflow's `ECR_REPOSITORY` — see Prerequisites |
| `temporal.image-tag` | `latest` | See Image tags below |
...
| `temporal.log-retention-days` | `14` | |
| `temporal.reviewer-email-to-github-login` | `{ "user@example.com": "example-login" }` | Reviewer-of-record's static tracker-email → GitHub-login table, passed through as `REVIEWER_EMAIL_TO_GITHUB_LOGIN`. A context value, not a secret. A mover missing from it gets no reviewer request and no revision rounds on their PRs — see `dispatch-worker/README.md` |
...
CI pushes every image under two tags: the commit's 12-character short sha and `latest`. The template
sets all three `image-tag` keys to `latest`, so a new build never needs a Terraform change — but
Terraform also sees nothing to plan, so a new image does not roll out by itself:

- **`listener` and `temporal-workers`** keep running the image they started with until the service is
  restarted — `aws ecs update-service … --force-new-deployment` (see Creating the SSM parameters above).
- **`specialist-sandbox`** has no long-running task; each dispatch's `RunTask` starts from whatever
  `latest` is at that moment.

A config change (a context value, a new env var) still needs an apply. When applying both, apply
`specialist-sandbox` before `temporal-workers`, since the worker reads its outputs. To make the running
version visible in git and in the plan, pin a sha tag in the key instead of `latest`; deploying a new
build is then: read the tag CI pushed, edit the key, apply.
...
repository and first image, its SSM parameters, and the Temporal Cloud admin API key all exist. It
reads `specialist-sandbox`'s outputs, so apply that stack first:
...
**`listener` depends on `temporal-workers` having deployed first** — it reads that stack's remote
...
secrets. On a from-scratch stand-up the order is `network`, `specialist-sandbox`, `temporal-workers`,
`listener` (`main.ts` declares these dependencies, so `npm run deploy` applies them in that order):
...
The shape follows the C# reference CDKTF project this was modelled on: context as the single configuration source, a base stack owning the provider and backend, typed
...
instead, one per stack whose outputs another stack reads. Writing one by hand for each boundary is
part of why the stacks are split only by rate of change and not further.
...
- **Specialist-sandbox egress is unrestricted, not allowlisted (parked).** The design calls for locking egress to
  ...
  with zero inbound.
- **The specialist sandbox holds the listener's own credentials.** It reads `ANTHROPIC_API_KEY`,
  `LINEAR_AGENT_API_KEY` and `GITHUB_TOKEN` directly via its execution role, from the same SSM
  parameters under the same shared `parameter-prefix` as the listener — not sandbox-scoped copies. A
  specialist run therefore holds the same raw credentials, with the same reach, as the rest of the
  deployment. This is the accepted current state. The design's credential-injection proxy (outside the
  sandbox, injecting credentials into requests so the container never holds one) is parked together
  with the egress allowlist, to be reopened before an engagement whose security review requires them.
...
- **Temporal worker egress is unrestricted**, same reasoning and same parked gap as the specialist
  sandbox's.
- **A new image doesn't roll out on its own.** With `image-tag` set to `latest`, the listener and
  worker services keep their running image until restarted — see Image tags.
